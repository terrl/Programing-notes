...
int ChessBoard::getStoneCountAtLine(int row1, int col1, int row2, int col2)
{
    ...
    if(row1 == row2)
    {
        ...
        {
            if(getStoneId(row1, col) != -1) ++ret;//中间有几颗棋子
        }
    }
    else
    {
...
void ChessBoard::whoWin()  //谁胜谁负
{
    if(m_ChessPieces[4].m_bDead == true && m_ChessPieces[20].m_bDead == false)
    {
        ...
        QMessageBox message(QMessageBox::Information, "提示", "本局游戏结束，红方胜利.");
    ...
    if(m_ChessPieces[4].m_bDead == false && m_ChessPieces[20].m_bDead == true)
    {
        ...
        QMessageBox message(QMessageBox::Information, "提示", "本局游戏结束，黑方胜利.");
        message.setIconPixmap(QPixmap(":/images/win.jpg"));
        message.setFont(QFont("华文行楷",16,QFont::Bold));
        message.exec();
    }
...
bool ChessBoard::isChecked(QPoint pt, int &row, int &col)
{
    for(row = 0; row <= 9; row++)
    {
        for(col = 0; col <= 8; col++)
        {
            ...
            if(x*x+y*y < m_nR*m_nR)//在棋子大小范围内，选中棋子
                return true;
        }
    }
...
QPoint ChessBoard::center(int row, int col)
{
    QPoint rePoint;
    //这里注意坐标的转换
    rePoint.ry() = row*m_nD+m_nOffSet;//棋子在界面上的坐标
...
void ChessBoard::paintEvent(QPaintEvent *)
{
    ...
    painter.scale(side / 960.0, side / 960.0);
        ...
        for(int i = 0; i <= 8; i++)
        {
            ...
            {
                painter.drawLine(QPoint(m_nOffSet+i*m_nD, m_nOffSet), QPoint(m_nOffSet+i*m_nD, m_nOffSet+4*m_nD));//楚河汉界下4上4
        ...
        QRect rect1(m_nOffSet+m_nD,   m_nOffSet+4*m_nD, m_nD, m_nD);//汉字位置
        ...
        painter.setFont(QFont("楷体", m_nR, 800));
...
void ChessBoard::drawChessPieces(QPainter &painter, int id)   //绘画单个具体的棋子
{
    ...
    if(m_nSelectID == id)
        painter.setBrush(QBrush(QColor(255,192,203,200)));//选中颜色显示
    else
        painter.setBrush(QBrush(QColor(107,142,35,40)));

    painter.setPen(QColor(165,42,42));//圆弧颜色
...
bool ChessBoard::canMoveJIANG(int moveId, int killId, int row, int col)
{
    ...
    {
        if(row < 7 || col < 3 || col > 5) return false;//从0开始，将在3/4/5列，7/8/9行
    }
    else  //黑 将
    {

        if(row > 2 || col < 3 || col > 5) return false;//3/4/5列，0/1/2行
    }

    int dr = m_ChessPieces[moveId].m_nRow - row;
    int dc = m_ChessPieces[moveId].m_nCol - col;
    int d = abs(dr)*10 + abs(dc);
    if( d == 1 ||d == 10)//同一行或同一列移动且只能移动一格
        return true;
