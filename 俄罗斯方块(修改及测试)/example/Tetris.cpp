...
// 全局变量: 
HINSTANCE hInst;                               // 定义HINSTANCE型变量
WCHAR szTitle[MAX_LOADSTRING];                  // 定义WCHAR型变量
WCHAR szWindowClass[MAX_LOADSTRING];            // 定义WCHAR型变量
HMENU diff;//定义菜单句柄变量
HMENU lay;//定义菜单句柄变量
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	WNDCLASSEXW wcex;//创建对象

	wcex.cbSize = sizeof(WNDCLASSEX);//为对象申请空间
	wcex.style = CS_HREDRAW | CS_VREDRAW;//
	wcex.lpfnWndProc = WndProc;//
	wcex.cbClsExtra = 0;//
	wcex.cbWndExtra = 0;//
	wcex.hInstance = hInstance;//
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_TETRIS));//设置图标
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);//加载光标
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);//加载背景
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_TETRIS);//加载选项
	wcex.lpszClassName = szWindowClass;//加载难度等级
	wcex.hIconSm = LoadIcon(wcex.hInstance, MAKEINTRESOURCE(IDI_SMALL));//

	return RegisterClassExW(&wcex);//反馈玩家选择
}
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	ShowWindow(hWnd, nCmdShow);//在窗口显示
